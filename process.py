...
matchurl = [re.compile(r"https?://\S+"), re.compile(r"[a-z]+\.[a-z/]+")]
# Subtract length of @auhansard_said for replies
tweet_len = 280 - 16
...
def process_text(text, house, dateseen, speaker, csv_writer):
    ...
    for sentence in sentences:
        ...
        for word in words:
            ...
            if (
                not skip_word(word)
                and checkword not in newwords
                and not r.get(checkword)
            ):
                if do_store:
